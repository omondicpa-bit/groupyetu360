...
async function loadMemberPendingFines(memberId){
  ...
  try{
    ...
    if(alertEl){
      ...
      if(pending.length&&alertText)alertText.innerHTML=`You have <strong>${pending.length} outstanding fine${pending.length!==1?'s':''}</strong> totalling <strong style="color:var(--maroon)">Ksh ${total.toLocaleString()}</strong>.<br><span style="font-size:.78rem;color:var(--ink-faint)">${pending.map(f=>h(f.reason)).join(' · ')}</span>`;
    }
